local AbstractLenCell, parent = torch.class('srnn.AbstractLenCell', 'srnn.LenRecurrentContainer')

function AbstractLenCell:__init(...)
...
function AbstractLenCell:prepareForward(input)
	...
	if self.backwarded then
		...
		if self.train then
			...
			self.farGradWeight:resize(self.nlayer * 2, bsize, self.outputSize):zero()
...
function AbstractLenCell:clearState()
...
function AbstractLenCell:getInput(step, input)
...
function AbstractLenCell:getGradOutput(step, gradOutput, lastStep, input)
	local function buildTable(tba, tbta)
		for _, unit in ipairs(tbta) do
			table.insert(tba, unit)
		end
	end
	local function stepAccResetGrad(gradIn, gradTar, lenvec, curstep, sind, ind, bsize)
		local _rv = rv or 0
...
function AbstractLenCell:getStepCell(step, layer)
	return self:net(step).output[self.nlayer + (layer or self.nlayer)]
end

function AbstractLenCell:setStepGradCellAdd(gradToAdd, step, layer)
...
function AbstractLenCell:setLayerInitCellState(statein, layer)
	self.initStates[self.nlayer + (layer or self.nlayer)] = statein
end

function AbstractLenCell:getLayerGradInitCellState(layer)
